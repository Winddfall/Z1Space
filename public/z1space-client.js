(() => {
  ...
  persist = function () {
    ...
    if ((snapshot.profileConfirmedAt || snapshot.step === 'done') && Array.isArray(snapshot.impressions) && snapshot.impressions.length === 3) {
  ...
  function a2aMessagesPanel() {
    ...
    return `<section class="agent-message-panel agent-a2a-panel"><div class="agent-panel-head"><button class="text-button" data-action="agent-chat-back">← 返回任务结果</button><span class="agent-live-dot">${esc(a2aStatusLabels[session.status] || 'Agent 交流')}</span></div><div class="agent-person-head"><div class="agent-person-avatar">${esc(String(person.name || '同').slice(0, 1))}</div><div><span class="eyebrow">A2A FIRST CHAT · HTTP/JSON-RPC</span><h2>你的 Agent × ${esc(person.name || '同路人')} 的 Agent</h2><p>${esc(person.role || '知乎公开用户')} · 已进行 ${Number(session.currentRound || 0)} / 3 轮</p></div></div><div class="agent-context"><span>共同话题</span><b>${esc(session.topic || person.topic || '围绕 Skill 继续交流')}</b></div><div class="agent-a2a-turns">${turns.length ? turns.map(turn => `<div class="agent-a2a-turn ${turn.speaker === 'requester_agent' ? 'mine' : ''}"><small>${turn.speaker === 'requester_agent' ? '你的 Agent' : `${esc(person.name || '同路人')} 的 Agent`} · 第 ${Number(turn.round || 0)} 轮</small><p>${esc(turn.text || '')}</p></div>`).join('') : '<div class="agent-empty">双方 Agent 正在准备第一轮交流…</div>'}</div>${observation ? `<div class="agent-a2a-observation"><span class="eyebrow">OBSERVER 结论</span><h3>${esc(verdict)}</h3><p>${esc(observation.reason || '')}</p>${observation.suggestedOpening ? `<div class="agent-context"><span>建议开场</span><b>${esc(observation.suggestedOpening)}</b></div>` : ''}</div>` : ''}${session.failureCode ? `<div class="agent-empty">${esc(a2aErrorMessage(session.failureCode, '双方 Agent 暂时未能完成交流。'))}</div>` : ''}</section>`;
  }
  function agentMessagesPanel() {
    ...
    return `<section class="agent-message-panel"><div class="agent-panel-head"><div><span class="eyebrow">AGENT TASK</span><h2>${esc(run.skill?.name || '最近一次探索')}</h2></div><span class="agent-status-pill ${statusClass}">${statusLabel}</span></div><div class="agent-timeline">${stages.map((text, i) => `<div class="agent-stage ${i < (run.stage || 0) ? 'done' : i === (run.stage || 0) ? 'current' : ''}"><span>${i < (run.stage || 0) ? '✓' : i + 1}</span><p>${text}</p></div>`).join('')}</div>${run.timeline?.length ? `<div class="agent-summary">${esc(run.timeline[run.timeline.length - 1].text)}</div>` : ''}${candidates.length ? `<div class="agent-results-head"><div><span class="eyebrow">MATCHED PEOPLE</span><h3>这些人，值得先聊聊</h3></div><span class="muted">${candidates.length} 位候选人</span></div><div class="agent-person-grid">${candidates.map(p => { const a2aEligible = p.a2aEligible !== false; return `<article class="agent-person-card"><div class="agent-card-top"><div class="agent-person-avatar">${esc(p.name.slice(0, 1))}</div><div><h3>${esc(p.name)}</h3><p>${esc(p.role)}</p></div></div><div class="agent-tags">${p.tags.map(tag => `<span>${esc(tag)}</span>`).join('')}</div><p class="agent-reason">${esc(p.reason)}</p><button class="button small wide" data-action="agent-chat" data-person="${p.id}" data-run-id="${esc(run.id)}" data-a2a-eligible="${a2aEligible ? 'true' : 'false'}">${a2aEligible ? '让双方 Agent 先聊聊' : '查看公开资料 Agent'}</button></article>`; }).join('')}</div>` : `<div class="agent-empty">${emptyMessage}</div>`}</section>`;
  }
  ...
  async function openAgentChat(personId, runId, a2aEligible = true) {
    try {
      ...
      if (!a2aEligible) { await openDirectChat(); return; }
  ...
  document.addEventListener('click', e => {
    ...
    if (b) openAgentChat(b.dataset.person, b.dataset.runId, b.dataset.a2aEligible !== 'false');
