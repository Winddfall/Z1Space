...
function advance(run: Run) {
  ...
  while (run.stage < stage) {
    ...
    if (run.stage === 3) { run.status = 'completed'; run.timeline.push({ kind: 'agent', text: completionSummary(run) }); }
  }
  return run;
}
