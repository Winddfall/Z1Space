...
async function enrichRun(run: Run) {
  ...
  try {
    ...
    for (const result of results) {
      ...
      const person = run.people[id];
      dynamicPeople.set(id, person);
      candidateProfileProvider.setProfile(id, buildAgentContextSnapshot({
        profileVersion: 1,
        ...
        profileSourceReferences: [[person.url || `candidate:${id}:role`], [person.url || `candidate:${id}:bio`], [person.url || `candidate:${id}:topic`]],
