...
export function recallPeople(candidates: readonly PeopleCandidate[], query: string, profile: AgentContextSnapshot, limit = 10): ExploreResult<PeopleCandidate> {
  ...
  return Object.freeze({
    ...
    recommendations: Object.freeze(found.map(candidate => {
      ...
      const candidateTopics = [...candidate.tags, ...(candidate.topic ? [candidate.topic] : [])];
      return recommendation('person', candidate.id, query, `${candidate.name} ${candidate.role} ${candidate.bio} ${candidate.tags.join(' ')} ${candidate.topic || ''}`, candidateTopics, [candidate.role, candidate.bio], divergenceSignals, profile, [...profileEvidence, candidateEvidence]);
    }))
  });
}
